...
resource "null_resource" "openssl_commands" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      openssl genpkey -algorithm RSA -out "${var.key_protect_path}/KPClient.key"
      openssl req -new -key "${var.key_protect_path}/KPClient.key" -out "${var.key_protect_path}/KPClient.csr" -subj "/C=US/ST=New York/L=Armonk/O=International Business Machines Corporation/CN=strgscale.com"
      openssl x509 -req -days 365 -in "${var.key_protect_path}/KPClient.csr" -signkey "${var.key_protect_path}/KPClient.key" -out "${var.key_protect_path}/KPClient.cert"
    EOT
  }
}
